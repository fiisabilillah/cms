    ...
    public function berita()
    {
        ...
        if ($keywordcari) {
            if ($jenis == 'berita') {
                ...
                $list =  $this->layanan->cari($keywordcari);
                $nmfile =  'v_hasilcarilayanan';
                $pno =  6;
            } else if ($jenis == 'agenda') {
                $list =  $this->agenda->cari($keywordcari);
                $nmfile =  'v_hasilcariagenda';
                $pno =  6;
            } else if ($jenis == 'bankdata') {
                $list =  $this->bankdata->cari($keywordcari);
                $nmfile =  'v_hasilcaribankdata';
                $pno =  6;
            } else if ($jenis == 'pengumuman') {
                $list =  $this->layanan->caripengumuman($keywordcari);
                $nmfile =  'v_hasilcaripengumuman';
                $pno =  6;
            } else if ($jenis == 'produkhukum') {
                $list =  $this->produkhukum->cari($keywordcari);
                $nmfile =  'v_hasilcariprohukum';
                $pno =  6;
            } else if ($jenis == 'infografis') {
                $list =  $this->banner->cari($keywordcari);
                $nmfile =  'v_hasilcariinfografis';
                $pno =  6;
            } else if ($jenis == 'albumfoto') {
                $list =  $this->kategorifoto->cari($keywordcari);
                $nmfile =  'v_hasilfoto';
                $pno =  12;
            } else if ($jenis == 'video') {
                $list =  $this->video->cari($keywordcari);
                $nmfile =  'v_hasilcarivideo';
                $pno =  6;
            }
            ...
            if ($template['duatema'] == 1) {
                $agent = $this->request->getUserAgent();
                if ($agent->isMobile()) {
                    return view('frontend/' . $template['folder'] . '/mobile/' . 'content/' . $nmfile, $data);
                } else {
                    return view('frontend/' . $template['folder'] . '/desktop/' . 'content/' . $nmfile, $data);
                }
            } else {
                return view('frontend/' . $template['folder'] . '/desktop/' . 'content/' . $nmfile, $data);
            }
